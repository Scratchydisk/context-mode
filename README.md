...
- **`CONTEXT_MODE_DISABLE_WEBFETCH_REDIRECT` opt-out** ([upstream issue #1003](https://github.com/mksglu/context-mode/issues/1003), still open) — `WebFetch`'s deny-and-redirect was the only routing intervention with no env-var opt-out. A model that can't act on the deny (behind a small local model, or when the deny loses a race against a successful sibling call in the same parallel tool batch) loses the fetch entirely instead of getting redirected — degrading the answer rather than saving context. `CONTEXT_MODE_DISABLE_WEBFETCH_REDIRECT=1` passes WebFetch through untouched, scoped to WebFetch alone.
